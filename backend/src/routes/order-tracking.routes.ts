...
const blockBaytWarehouseWrites = (req: Request, res: Response, next: NextFunction) => {
  if (config.erpProvider === 'bayt') {
    return res.status(409).json({
      error: 'Bayt ERP read-only modunda depo/siparis operasyon yazmalari desteklenmiyor.',
      code: 'ERP_READ_ONLY',
    });
  }
  return next();
...
router.post('/admin/sync', authenticate, requirePermission('admin:order-tracking'), blockBaytWarehouseWrites, orderTrackingController.syncPendingOrders);
router.post('/admin/send-emails', authenticate, requirePermission('admin:order-tracking'), blockBaytWarehouseWrites, orderTrackingController.sendEmails);
router.post('/admin/send-customer-emails', authenticate, requirePermission('admin:order-tracking'), blockBaytWarehouseWrites, orderTrackingController.sendCustomerEmails);
router.post('/admin/send-supplier-emails', authenticate, requirePermission('admin:order-tracking'), blockBaytWarehouseWrites, orderTrackingController.sendSupplierEmails);
router.post('/admin/supplier-transmissions/:customerCode', authenticate, requirePermission('admin:order-tracking'), blockBaytWarehouseWrites, orderTrackingController.markSupplierTransmitted);
router.post('/admin/send-email/:customerCode', authenticate, requirePermission('admin:order-tracking'), blockBaytWarehouseWrites, orderTrackingController.sendEmailToCustomer);
router.post('/admin/sync-and-send', authenticate, requirePermission('admin:order-tracking'), blockBaytWarehouseWrites, orderTrackingController.syncAndSend);
...
router.post('/admin/warehouse/sync', authenticate, requirePermission('admin:order-tracking'), blockBaytWarehouseWrites, orderTrackingController.syncPendingOrders);
...
router.post('/admin/warehouse/orders/:mikroOrderNumber/start', authenticate, requireAnyPermission(['admin:order-tracking', 'admin:warehouse-kiosk']), blockBaytWarehouseWrites, warehouseWorkflowController.startPicking);
router.patch('/admin/warehouse/orders/:mikroOrderNumber/items/:lineKey', authenticate, requireAnyPermission(['admin:order-tracking', 'admin:warehouse-kiosk']), blockBaytWarehouseWrites, warehouseWorkflowController.updateItem);
router.post('/admin/warehouse/orders/:mikroOrderNumber/items/:lineKey/report-image-issue', authenticate, requireAnyPermission(['admin:order-tracking', 'admin:warehouse-kiosk']), blockBaytWarehouseWrites, warehouseWorkflowController.reportImageIssue);
router.post('/admin/warehouse/orders/:mikroOrderNumber/loaded', authenticate, requireAnyPermission(['admin:order-tracking', 'admin:warehouse-kiosk']), blockBaytWarehouseWrites, warehouseWorkflowController.markLoaded);
router.post('/admin/warehouse/orders/:mikroOrderNumber/dispatched', authenticate, requireAnyPermission(['admin:order-tracking', 'admin:warehouse-kiosk']), blockBaytWarehouseWrites, warehouseWorkflowController.markDispatched);
...
router.post('/admin/warehouse/retail/sales', authenticate, requireAnyPermission(['admin:order-tracking', 'admin:warehouse-retail']), blockBaytWarehouseWrites, warehouseWorkflowController.createRetailSale);
